...
export const DeleteClassModal: React.FC<DeleteClassModalProps> = ({ cls, onClose, onConfirm }) => {
  if (!cls) return null;
    ...
    <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-slate-900 border border-slate-700/80 rounded-2xl w-full max-w-md overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-150">
        <div className="p-6 text-center space-y-4">
          ...
          <div>
            <h3 className="text-base font-extrabold text-white">Xác Nhận Xóa Lớp Học?</h3>
            <p className="text-xs text-slate-400 mt-1">
              Bạn có chắc chắn muốn xóa lớp <strong className="text-white">{cls.name}</strong> ({cls.code}) khỏi hệ thống trung tâm? Hành động này không thể hoàn tác.
            </p>
          </div>
